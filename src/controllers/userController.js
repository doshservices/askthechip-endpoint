...
module.exports.requestMentorship = async (req, res) => {
  try {
    const mentorship = await new User(req.body).requestMentorship();
    return success(res, { mentorship }, "Request Sent");
  } catch (err) {
    logger.error("Error occurred at requestMentorship", err);
    return error(res, { code: err.code, message: err.message });
  }
};
...
module.exports.subscribeUser = async (req, res) => {
  try {
    ...
    const subcribe = await new User({ email, userId }).subscribeUser();
    return success(res, { subcribe }, "Request Sent");
  } catch (err) {
    logger.error("Error occurred at subscribeUser", err);
    return error(res, { code: err.code, message: err.message });
  }
};
